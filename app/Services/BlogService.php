...
class BlogService extends BaseController
{
    ...
    public function listService($arrParam=array())
    {
        try{
            ...
            if(isset($arrParam['categoria']) ) {
                $sufix .= "&categorias=" . $arrParam['categoria'];
            }
            
            $param              = array( 
                'url'           => env("API_PATH")."/".env("API_KEY")."/pagina?" . $sufix, 
            );   
    ...
    public function listCategoriaService()
    {
        try{
            ...
            // 5 é a categoriaPai Blog
            $param              = array( 
                'url'           => env("API_PATH")."/".env("API_KEY")."/pagina/categoria?categoriaPai=2", 
            );      
    ...
    public function getDestaks($qtd=6, $catId=false)
    {
        ...
        if ($catId) {
            ...
            $destak = $this->listService(["pagesize"=>100, "categoria"=>2]);
        }
    ...
    public function viewService($arrParam=array())
    {
        try{
            ...
            $param              = array( 
                'url'           => env("API_PATH")."/".env("API_KEY")."/Pagina/Detalhe/".$NoticiaID, 
            );    
